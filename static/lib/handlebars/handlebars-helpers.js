...
//format a number with numeral.js
Handlebars.registerHelper('numeral', function(num, format) {
	return new Handlebars.SafeString(numeral(num).format(format));
});
...
Handlebars.registerHelper('when', function(value, options) {
	...
	} else if ('gt' in options.hash) {
		if (value <= options.hash.gt) {
			return options.inverse(this);
		} else {
			return options.fn(this);
		}
	}else if ('lt' in options.hash) {
		if (value >= options.hash.lt) {
			return options.inverse(this);
		} else {
			return options.fn(this);
		}
	} else if ('gte' in options.hash) {
		if (value < options.hash.gte) {
			return options.inverse(this);
		} else {
			return options.fn(this);
		}
	}else if ('lte' in options.hash) {
		if (value > options.hash.lte) {
			return options.inverse(this);
		} else {
			return options.fn(this);
		}
